...
class MCPClient:
    ...
    async def process_query(self, query: str) -> str:
        ...
        template_info = await self.session.read_resource("mcp://resource-templates")
        templates_data = template_info.contents[0].text
        
        available_tools.append({
            "name": "read_resource",
            "description": f"Read an MCP resource. Available resource URI templates:\n{templates_data}",
            "input_schema": {
                "type": "object",
                "properties": {
                    "uri": {
                        "type": "string",
                        "description": "The resource URI. Replace {parameter} with actual values."
                    }
                },
                "required": ["uri"]
            }
